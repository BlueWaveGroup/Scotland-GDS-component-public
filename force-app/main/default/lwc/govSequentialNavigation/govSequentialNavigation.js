/**
 * Component Name: Gov Sequential Navigation
...
export default class GovSequentialNavigation extends LightningElement {
    // flow actions
    ...
    @api buttonTextString;
    @api buttonAlignmentsString;
    @api action;
    @api fieldId = 'govSequentialNavigation';
    @api useFlowStyling;
    @api fullWidth = false;
    ...
    connectedCallback() {
        ...
        if(this.availableActions.find(action => action === 'FINISH')){
            // on last screen - hide 'NEXT' button
            this.displayRightButton = false;
        }
        if(!this.availableActions.find(action => action === 'NEXT')){
            //console.log('On last screen');
        }
        if(!this.availableActions.find(action => action === 'BACK')){
            // on first screen - hide 'PREVIOUS' option
        ...
        const buttonText = this.buttonTextString.split(',');
        ...
        for(let i=0; i<buttonAlignments.length; i++) {
            ...
            if(button.variant.toUpperCase() === 'BRAND') {
                button.class = 'ds_button govuk-!-margin-1';
            } else if(button.variant.toUpperCase() === 'SECONDARY') {
                button.class = 'ds_button ds_button--secondary govuk-!-margin-1';
            } else if(button.variant.toUpperCase() === 'WARNING') {
                button.class = 'ds_button govuk-button--warning govuk-!-margin-1';
            } else if(button.variant.toUpperCase() === 'DISABLED'){
                button.class = 'ds_button govuk-button--disabled govuk-!-margin-1';
            } else {
                button.class = 'ds_button govuk-!-margin-1';
            }
    ...
    handleClick(event) {
        ...
        console.log(`GOV SEQUENTIAL NAVIGATION: components are ${JSON.stringify(this.components)}`);
        ...
        if( (this.action === 'NEXT' || this.action === 'FINISH') && this.components.length > 0 ) {
            ...
            console.log('GOV SEQUENTIAL NAVIGATION: Sending validation message');
    ...
    handleValidationUpdate(message) {
        ...
        if(invalidComponents.length === 0) {
            ...
            console.log(`GOV SEQUENTIAL NAVIGATION: There are invalid components.`);
        }
    }
}